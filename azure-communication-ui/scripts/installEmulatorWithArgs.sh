...
# Create emulators
echo "no" | $ANDROID_HOME/tools/bin/avdmanager create avd -n "$1" -k ${SYSTEM_IMAGE} --force --device "pixel_xl" --tag "${SYS_IMG_TAG}"

echo "avdmanager successfully created $($ANDROID_HOME/emulator/emulator -list-avds)"
echo "Starting emulator"

sed -i'' -e "s%android-sdk/system-images/${OS_VERSION}/${SYS_IMG_TAG}/${CPU}/%system-images/${OS_VERSION}/${SYS_IMG_TAG}/${CPU}/%g" ~/.android/avd/"$1".avd/config.ini
sed -i'' -e "s%hw.keyboard=no%hw.keyboard=yes%g" ~/.android/avd/"$1".avd/config.ini


#$PWD/android-sdk/platform-tools/adb kill-server

# Start emulator in background 
$ANDROID_HOME/emulator/emulator @"$1" -no-snapshot -skin 1130x2360 -accel auto -gpu auto -screen touch -camera-back emulated -camera-front emulated -use-host-vulkan -dns-server 8.8.8.8  &
